  ...
  return (
    <div className="px-6 md:px-12 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col lg:flex-row gap-16 xl:gap-24 items-start">
        ...
        <div className="w-full lg:w-3/5 bg-surface-container-low p-6 md:p-12">
          <div className="relative overflow-hidden bg-surface-container-lowest p-6">
            <Image
        ...
        <div className="w-full lg:w-2/5 flex flex-col space-y-12 lg:sticky lg:top-40">
          <section className="space-y-4">
            <p className="font-body text-xs tracking-[0.2em] uppercase text-secondary font-semibold">
              {artwork.album.title}
            </p>
            <h1 className="font-headline text-5xl md:text-6xl tracking-tighter leading-tight text-on-background">
              {artwork.title}
            </h1>
          </section>
          ...
          <div className="space-y-8">
            {(artwork.medium || dimensions || artwork.createdDate) && (
              <div className="grid grid-cols-2 gap-8 border-y border-on-surface/5 py-8">
                {artwork.medium && (
                  <div>
                    <p className="font-body text-[10px] tracking-widest uppercase text-on-surface-variant mb-1">Material / Technik</p>
                    <p className="font-body text-md text-on-surface">{artwork.medium}</p>
                  </div>
                )}
                {dimensions && (
                  <div>
                    <p className="font-body text-[10px] tracking-widest uppercase text-on-surface-variant mb-1">Maße</p>
                    <p className="font-body text-md text-on-surface">{dimensions}</p>
                  </div>
                )}
                {artwork.createdDate && (
                  <div>
                    <p className="font-body text-[10px] tracking-widest uppercase text-on-surface-variant mb-1">Entstehung</p>
                    <p className="font-body text-md text-on-surface">{artwork.createdDate}</p>
                  </div>
                )}
              </div>
            )}
